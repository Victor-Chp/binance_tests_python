...
def test_get_order_book(symbol):
    ...
    response = custom_session.get(f'{base_url.perpetual_futures}/{marketroutes.depth}', params=params)
...
def test_get_recent_trades_list(symbol):
    ...
    response = custom_session.get(f'{base_url.perpetual_futures}/{marketroutes.trades}', params=params)
    assert response.status_code == 200, f'Status code {response.status_code}, not 200'
...
def test_get_one_kline_perpetual_futures(symbol, interval):
    ...
    response = custom_session.get(f'{base_url.perpetual_futures}/{marketroutes.klines}', params=params)
    assert response.status_code == 200, f'Status code {response.status_code}, not 200'
